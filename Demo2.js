...
var w = (c.width = window.innerWidth),
  ...
  hh = h / 2,
  opts = {
    strings: ["Happy", "Birthday", "Sushmita Rai !!!!"],
    charSize: 50,
    charSpacing: 45,
    lineHeight: 50,
    ...
    balloonSpawnTime: 300,
    balloonBaseInflateTime: 300,
...
function generateBalloonPath(x, y, size) {
  ctx.moveTo(x, y);
  ctx.quadraticCurveTo(x - 3*size/2, y - 5*size/2, x, y - 3*size/2);
  ctx.quadraticCurveTo(x + 3*size/2, y - 5*size/2, x, y );
//   ctx.bezierCurveTo(
//     x - size / 2,
//     y - size / 2,
//     x - size / 4,
...
function anim() {
  ...
  // Uncomment the below line to start the loop.
...
window.addEventListener("resize", function () {
  ...
  ctx.font = opts.charSize + "px cursive";
});